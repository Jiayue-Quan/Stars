          ...
          <div className="flex flex-col gap-8 lg:flex-row">
            <div className="w-full max-w-xs flex-none">
              <div className="overflow-hidden rounded-[1.8rem] border border-white/10 bg-white/[0.03] shadow-[0_24px_70px_-28px_rgba(0,0,0,0.95)]">
                {person.imageUrl ? (<img src={person.imageUrl} alt={person.name} className="aspect-[4/5] w-full object-cover" loading="eager" decoding="async"/>) : (<div className="flex aspect-[4/5] items-center justify-center">
            ...
            <div className="max-w-4xl flex-1">
              ...
              <div className="section-panel mt-6 rounded-[1.5rem] p-6">
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-2">
                  {person.knownFor.length > 0 ? (person.knownFor.map((credit) => (<Link key={`${credit.mediaType}-${credit.id}`} to={getSearchResultHref({ id: credit.id, mediaType: credit.mediaType })} className="group flex items-center gap-3 rounded-[1.2rem] border border-white/8 bg-white/[0.03] p-3 transition-all hover:border-white/15 hover:bg-white/[0.05]">
                        <div className="flex h-16 w-12 flex-none items-center justify-center overflow-hidden rounded-xl border border-white/10 bg-white/[0.04]">
                          {credit.imageUrl ? (<img src={credit.imageUrl} alt={credit.title} className="h-full w-full object-cover" loading="lazy" decoding="async"/>) : (<UserRound className="h-5 w-5 text-muted-foreground"/>)}
                        </div>
                      ...
                      </Link>))) : (<p className="text-sm text-muted-foreground">Known-for credits are not available for this profile.</p>)}
                </div>
              </div>
