          ...
          <div className="flex flex-col gap-8 lg:flex-row">
            <div className="w-full max-w-xs flex-none">
              <div className="overflow-hidden rounded-[1.8rem] border border-white/10 shadow-[0_24px_70px_-28px_rgba(0,0,0,0.95)]">
                <PosterImage src={show.posterUrl} title={show.title} className="aspect-[2/3] w-full object-cover" width={500} height={750} loading="eager"/>
              </div>
            </div>
            ...
            <div className="max-w-3xl flex-1">
              ...
              <div className="section-panel mt-6 rounded-[1.4rem] p-5">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {show.cast.length > 0 ? (show.cast.map((member) => (<span key={member} className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1.5 text-sm text-white">
                        {member}
                      </span>))) : (<p className="text-sm text-muted-foreground">Cast information is not available for this series.</p>)}
                </div>
              </div>
