...
export function Watchlist() {
  ...
  const tabMovies = useMemo(() => {
    const activeIds =
      activeTab === 'watchlist' ? library.watchlist : activeTab === 'watched' ? library.watched : library.favorites;
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex gap-2 overflow-x-auto pb-2">
          {tabs.map((tab) => (
            <button
              ...
              className={`flex items-center gap-2 whitespace-nowrap rounded-full px-5 py-2.5 font-medium transition-all ${
                activeTab === tab.id ? 'bg-primary text-white' : 'bg-white/5 text-muted-foreground hover:bg-white/10'
              }`}
            >
        ...
        <div className="mb-8 flex flex-wrap items-center gap-4 rounded-2xl border border-white/[0.06] bg-card/40 p-4">
          ...
          <select value={selectedVerdict} onChange={(event) => setSelectedVerdict(event.target.value)} className="rounded-lg border border-white/10 bg-secondary/50 px-3 py-1.5 text-sm">
            ...
            {['Masterpiece', 'Essential', 'Recommended', 'Mixed', 'Skip'].map((verdict) => (
          ...
          <div className={`grid gap-4 ${viewMode === 'grid' ? 'grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5' : 'grid-cols-1'}`}>
            {filteredMovies.map((movie) =>
              viewMode === 'grid' ? (
                ...
                <div key={movie.id} onClick={() => navigate(`/review/${movie.id}`)} className="flex cursor-pointer gap-4 rounded-xl border border-white/[0.06] bg-card/40 p-4 hover:border-white/[0.12]">
