...
export function getPosterFallback(title, size = {}) {
    ...
    const titleMarkup = lines
        .map((line, index) => `<text x="50%" y="${textY + index * lineGap}" text-anchor="middle" fill="${FALLBACK_FG}" font-family="Oswald, Arial, sans-serif" font-size="24" letter-spacing="1">${escapeXml(line)}</text>`)
        .join("");
    const svg = `
...
function isLikelyValidTmdbPath(posterUrl) {
    try {
        const parsed = new URL(posterUrl);
        if (!parsed.hostname.includes("image.tmdb.org"))
            return true;
...
function isAmbiguousPosterUrl(posterUrl) {
    return AMBIGUOUS_POSTER_URLS.has(posterUrl);
}
export function resolvePosterUrl(posterUrl, title, size = {}) {
    if (!posterUrl)
        return getPosterFallback(title, size);
    if (!posterUrl.startsWith("http"))
        return posterUrl;
    if (isAmbiguousPosterUrl(posterUrl))
        return getPosterFallback(title, size);
    if (!isLikelyValidTmdbPath(posterUrl))
        return getPosterFallback(title, size);
    return posterUrl;
}
